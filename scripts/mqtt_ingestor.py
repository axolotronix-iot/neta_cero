...
class MQTTIngestor:
    ...
    Attributes:
        pg_config (dict): Configuración de conexión para asyncpg.
    ...
    def __init__(self, pg_config):
        ...
        Args:
            pg_config (dict): Diccionario que contiene las credenciales y parámetros 
                de conexión (host, database, user, password, port).
        """
    ...
    async def init_db(self):
        ...
        try:
            ...
            async with self.pool.acquire() as conn:
                ...
                print("[Init DB] DB pool creado correctamente")
        except Exception as e:
            print("[Init DB]Error inicializando DB:", e)
            raise
    ...
    async def mqtt_listener(self, reconnect_interval: int = 5, max_reconnect_interval: int = 60):
        ...
        while True:
            try:
                print(f"[MQTT Listener] Conectando al broker: {BROKER}...")
                async with Client(BROKER) as client:
                    print(f"[MQTT Listener] Conectado exitosamente a {BROKER}")
                ...
                print(f"[MQTT Listener] Error de conexión MQTT: {e}. Reintentando en {reconnect_interval}s...")
